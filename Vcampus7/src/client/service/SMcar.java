...
public class SMcar extends JPanel {
				...
				car.addMouseListener(new MouseAdapter() {
		 			...
		 			public void mouseClicked(MouseEvent e) {
		               ...
		               // search。setBorderPainted(false);//不绘制边框
			    		...
			    		Message msg = new Message("lookCar", obj);
			    		...
			    		for(int i=0;i<res.num;i=i+1)
			    		{
			    			...
			    			tableModel.addRow(new Object[]{((Goods)res.response[i]).name,((Goods)res.response[i]).id,String.valueOf(((Goods)res.response[i]).price),((Goods)res.response[i]).number,((Goods)res.response[i]).introduction,"移除"});
			    		...
			    		table.addMouseListener( new MouseListener(){
				             public void mouseClicked(MouseEvent e) {//仅当鼠标单击时响应
                               ...
                               if(c==5)
                               {
                            	   StringWithInt obj = new StringWithInt(value.toString(),id,1);
           			    		Message msg = new Message("deleteCar", obj);
           			    		Object temp = Sender.send(msg);
           			    		Message res = (Message)temp;
